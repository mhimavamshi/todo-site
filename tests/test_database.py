...
async def test_project_has_todos(session):
    ...
    new_todo2 = Todo(project_id=new_project.id, data="Test Todo 2", status=TodoStatus.completed)
    session.add(new_todo1)
    session.add(new_todo2)
    await session.commit()

    result = await session.execute(
        select(Project).filter(Project.id == new_project.id).options(selectinload(Project.todos))
    )
...
async def test_update_todo_status(session):
    ...
    new_todo = Todo(project_id=new_project.id, data="Test Todo", status=TodoStatus.in_progress)
    session.add(new_todo)
    await session.commit()
    await session.refresh(new_todo)
