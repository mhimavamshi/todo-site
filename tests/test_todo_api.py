...
async def project_id(client):
    response = client.post("/api/v1/projects/create", json={"title": "Test Project", "notes": "Test Notes"})
    assert response.status_code == 200
    return response.json()["id"]

@pytest_asyncio.fixture
async def todo_data(client, project_id):
    response = client.post("/api/v1/todos/project/{}/".format(project_id), json={"data": "Test Todo"})
    assert response.status_code == 200
    return response.json()  

def test_create_todo(client, project_id):
    response = client.post(f"/api/v1/todos/project/{project_id}/", json={"data": "New Todo"})
    assert response.status_code == 200
...
def test_update_todo(client, todo_data):
    ...
    response = client.patch("/api/v1/todos/", json={"id": todo_id, "status": "completed", "data": "Updated Todo"})
    assert response.status_code == 200
